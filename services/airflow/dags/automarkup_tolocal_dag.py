...
with DAG(
    dag_id="automarkup_tolocal",
    start_date=datetime(2024, 1, 1),
    schedule_interval="*/5 * * * *",
    catchup=False,
) as dag:
    tolocal_automarkup = DockerOperator(
        task_id="automarkup_to_local",
        image="diplom2-spark-master:latest",
        command="/opt/spark/bin/spark-submit --master spark://spark-master:7077 /app/automarkup_to_local.py",
